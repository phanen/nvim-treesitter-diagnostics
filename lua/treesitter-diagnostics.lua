...
---@param buf integer
local diagnose_node = function(node, parser, diagnostics, buf)
  ...
  for _, match in query:iter_matches(root, buf) do
    for _, nodes in pairs(match) do
      for _, node in ipairs(nodes) do
        diagnose_node(node, parser, diagnostics, buf)
      end
    end
  end
end
...
local diagnose_buffer = function(buf)
  if
    ...
    or next(vim.lsp.get_clients({ bufnr = buf, method = 'textDocument/diagnostic' }))
  then
    return
  end
